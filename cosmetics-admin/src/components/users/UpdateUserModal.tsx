...
export default function UpdateUserModal({
  ...
  const currentRoleIds: string[] = useMemo(() => {
    ...
    const r = user?.userRoles ?? [];
    return Array.isArray(r) ? r.map((x: any) => x.role.id) : [];
  }, [user]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // role update (single role select) -> assign/remove via endpoints
      ...
      for (const rid of toRemove) {
        await removeRole({ id: user.id, roleId: rid }).unwrap();
      }

      // assign selected if missing
      if (selectedRoleId && !oldRoleIds.includes(selectedRoleId)) {
        await assignRole({ id: user.id, roleId: selectedRoleId }).unwrap();
      }
  ...
  const busy = updating || assigning || removing;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-150">
        ...
        <DialogFooter className="gap-2">
          ...
          <Button onClick={handleUpdate} disabled={busy || !user}>
            Update
          </Button>
        </DialogFooter>
      </DialogContent>
