...
import TotalInvestment from "../Investment-submit/TotalInvestment";
import ReturnTotal from "../Investment-submit/ReturnTotal";

const ResaleInvestment = ({ projectData, investId }) => {
  ...
  const PreviousInvestmentParams = {
  ...
  const {
    ...
    isLoading,
  } = useGetSingleInvestmentByUserORInvestIdQuery(PreviousInvestmentParams);
  ...
  useEffect(() => {
    let profitAmount = 0;
    const countInvestment =
      projectData?.perShareValue * investData?.totalBuyShare;
      ...
      <div className="grid grid-cols-12 gap-6">
            ...
            <div className="flex flex-col justify-between">
              ...
              <div>
                <span className="text-sm lg:text-base text-[#068F66] font-semibold">
                  {investData?.totalBuyShare} Slots{" "}
                  <span className="text-black">
                    /{projectData?.perShareValue?.toLocaleString()} ={" "}
                    {formatBDT(investmentAmount)} (BDT)
                  </span>
                </span>
              </div>
        ...
        <div className="md:col-span-5 sm:col-span-12 pt-3">
          <TotalInvestment
            ...
            profitAmount={profitAmount}
            buyShare={investData?.totalBuyShare}
            allUsers={allUsers}
            setUserDataId={setUserDataId}
            userDataId={userDataId}
            nominees={userNominees}
            resaleInvestorUserId={investData?.userId?.id}
            isForResale={true}
          />
        </div>
      </div>
